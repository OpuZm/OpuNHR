...
//				projectAndDetailsAuto();//菜品父元素自适应
	...
	form.on('switch(IsRecommend)', function(data) {
		...
		OrderTableProjectsdata[index].IsRecommend = data.elem.checked;
	});
	
	//是否特价 单选按钮
	form.on('switch(IsSpecialOffer)', function(data) {
		var index = $(data.elem).closest('tr').attr('data-index');
		OrderTableProjectsdata[index].IsSpecialOffer = data.elem.checked;
	});
});
...
function AddSelect(pro) {
	...
	OrderTableProjectsdata[OrderTableProjectsdata.length - 1].IsSpecialOffer = OrderTableProjectsdata[OrderTableProjectsdata.length - 1].IsSpecialOffer > 0 ? true : false;
	OrderTableProjectsdata[OrderTableProjectsdata.length - 1].IsRecommend = OrderTableProjectsdata[OrderTableProjectsdata.length - 1].IsRecommend > 0 ? true : false;
	//更新订单
	NewsOrder()
}
...
function selectRecommend(){
	layer.confirm('是否确认对已选中菜品进行特价?',{icon: 3, title:'友情提示'}, function(index) {
		var tr = $('tr.layui-this',ProjectListsView);
		$.each(tr,function(){
			var index = $(this).attr('data-index');
			OrderTableProjectsdata[index].IsRecommend = true;
			$(this).find('input[name=IsRecommend]').prop('checked',true);
		})
		form.render('checkbox');
		layer.close(index);
	});
...
function selectSpecialOffer(){
	layer.confirm('是否确认对已选中菜品进行特价?',{icon: 3, title:'友情提示'}, function(index) {
		...
		$.each(tr,function(){
			var index = $(this).attr('data-index');
			OrderTableProjectsdata[index].IsSpecialOffer = true;
			$(this).find('input[name=IsSpecialOffer]').prop('checked',true);
		})
		form.render('checkbox');
		layer.close(index);
	});
...
//	$('#ProjectAndDetails_view li').removeClass('layui-this');
	...
	if(classno == 0){ //如果  当前   1级分类是   全部
		for(var j=0;j<inidata.ProjectAndDetails.length;j++){
			...
			if(type == 0 ? item.IsRecommend > 0 : item.IsSpecialOffer > 0){
				item.T_isSelected = true; //是否选中
...
function submitOrderTable(){
	...
	var para = [];
	for (let i = 0; i<OrderTableProjectsdata.length; i++) {
		let o = OrderTableProjectsdata[i];
		para.push({Id:o.Id,IsRecomand:o.IsRecommend,IsSpecialOffer:o.IsSpecialOffer})
	}
	...
	layer.confirm('是否确认提交?',{icon: 3, title:'友情提示'}, function(index) {
		...
		$.ajax({
            type: "post",
            url: "/Res/Project/ProjectRecommendSubmit",
            dataType: "json",
            data: JSON.stringify({req:para}),
...
//阻止启用库存按钮冒泡事件
$(document).on('click','#ProjectLists_view .layui-form-switch',function(e){
	e.stopPropagation();
})
