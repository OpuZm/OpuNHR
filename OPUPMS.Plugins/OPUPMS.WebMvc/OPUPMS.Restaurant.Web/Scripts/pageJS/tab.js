layui.define(['element', 'common'], function(exports) {
	...
	Tab.prototype.tabAdd = function(data) {
		...
		if(tabIndex === -1) {
			//globalTabIdIndex++;
			globalTabIdIndex = $('.layui-tab-title li').length;
			console.log(globalTabIdIndex);
			
			var content = '<iframe src="' + data.href + '" id="page_'+ data.id +'" data-id="' + globalTabIdIndex + '"></iframe>';
			var title = '';
			...
			if(_config.closed) {
				title += '<i class="layui-icon layui-unselect layui-tab-close" data-fid="page_'+ data.id +'" data-navid="'+ data.id +'" data-id="' + globalTabIdIndex + '">&#x1006;</i>';
			}
			//添加tab
			...
			var tab_nav='<li class="layui-this">'+title+'</li>';
			var tab_content = '<div class="layui-tab-item layui-show">'+content+'</div>';
			...
			if(_config.closed) {
				...
				ELEM.titleBox.find('li').children('i.layui-tab-close[data-id=' + globalTabIdIndex + ']').on('click', function() {
					...
					$(this).parent('li').remove();
                     $('#'+$(this).attr('data-fid')+'').parent('.layui-tab-item').remove();
			...
			var navno=globalTabIdIndex;
            $('.layui-tab-title li i.layui-tab-close[data-id=' + navno + ']').parent('li').addClass('layui-this').siblings("li").removeClass('layui-this');
            $('#page_'+data.id+'').parent('.layui-tab-item').addClass('layui-show').siblings(".layui-tab-item").removeClass('layui-show');
		} else {
			//element.tabChange(ELEM.tabFilter, tabIndex);
			var navno=tabIndex;
			
			var div=$('.layui-tab-title li i.layui-tab-close[data-id=' + navno + ']');
            div.parent('li').addClass('layui-this').siblings("li").removeClass('layui-this');
            $('#'+div.attr('data-fid')+'').parent('.layui-tab-item').addClass('layui-show').siblings(".layui-tab-item").removeClass('layui-show');
            
            var tab_nav='<li class="layui-this">'+$('.layui-tab-title li.layui-this').html()+'</li>';
			var tab_content = '<div class="layui-tab-item layui-show">'+$('#'+div.attr('data-fid')+'').parent('.layui-show').html()+'</div>';
			$.cookie('tab_nav', tab_nav, { path:'/' });
			$.cookie('tab_content', tab_content, { path:'/' });
