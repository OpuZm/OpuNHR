...
namespace OPUPMS.Infrastructure.Common
{
    ...
    public class ValidateExtend
    {
        ...
        {
            bool result = Regex.IsMatch(val, @"^[1-9]\d*\.?[0]*$");
            return result;
        }
        ...
        /// <returns>返回验证结果；true表示匹配成功，false表示匹配失败</returns>
        public static bool RegexMatch(string input, string pattern)
        {
            return RegexMatch(input, pattern, RegexOptions.None);
        }
        ...
        /// <returns>返回验证结果；true表示匹配成功，false表示匹配失败</returns>
        public static bool RegexMatch(string input, string pattern, RegexOptions options)
        {
            if (input.IsEmpty())
            {
                return false;
        ...
        /// 验证一个文件名是否为图片格式
        /// </summary>
        /// <param name="input">需要验证的文件名</param>
        /// <returns>返回验证结果；true表示合法，false表示非法</returns>
        public static bool IsImage(string input)
        {
            return RegexMatch(input, @"^.+\.(jpg|bmp|gif|png|jpeg)$", (RegexOptions.CultureInvariant | RegexOptions.IgnoreCase));
        }
        #endregion
    }
}
