...
{
    /// <summary>
    /// Web 页面处理帮助类，如Url及字符串的解析、编码，Seesion、Cookie的操作等
    /// </summary>
    public class WebHelper
    {
        ...
        public static void WriteAuthCookie(string strName, string strValue, int expires)
        {
            HttpCookie cookie = HttpContext.Current.Request.Cookies[strName];
            //if (cookie == null)
            //{
            ...
            FormsAuthentication.SetAuthCookie(strName, false, FormsAuthentication.FormsCookiePath);
            FormsAuthenticationTicket ticket = new FormsAuthenticationTicket(2, strName, nowTime, nowTime.AddHours(expires), false, strValue);
            
            cookie = new HttpCookie(strName, FormsAuthentication.Encrypt(ticket));
            cookie.Domain = FormsAuthentication.CookieDomain;
            cookie.Path = FormsAuthentication.FormsCookiePath;
            cookie.HttpOnly = true;
            //cookie.Value = strValue;
            cookie.Expires = nowTime.AddHours(expires);
            HttpContext.Current.Response.AppendCookie(cookie);
        }
        ...
        /// 获取授权信息
        /// </summary>
        /// <returns>获取授权用户信息</returns>
        public static string GetAuthCookie(string formAuthName)
        {
            //var cookie = HttpContext.Current.Request.Cookies[FormsAuthentication.FormsCookieName];
            var cookie = HttpContext.Current.Request.Cookies[formAuthName];
            if (cookie != null)
            {
