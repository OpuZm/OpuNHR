...
layui.define(["laytpl", "laypage", "layer", "form"], function(e) {
	...
	A.prototype.config = {
		...
		try {
			...
			var m = u.layHeader.find("th .laytable-cell-" + f.index + "-" + r).find(w);
			u.layHeader.find("th").find(w).removeAttr("lay-sort"), m.attr("lay-sort", i || null), u.layFixed.find("th")
		} catch(v) {
		...
		u.sortKey = {
			field: r,
			sort: i
		}, "asc" === i && 'Sorted' == r ? c = p : "asc" === i ? c = layui.sort(p, r) : "desc" === i && 'Sorted' == r ? c = p : "desc" === i ? c = layui.sort(p, r, !0) : (c = layui.sort(p, d.config.indexName), delete u.sortKey), h[f.response.dataName] = c, u.renderData(h, u.page, u.count, !0), l.close(u.tipsIndex), n && layui.event.call(e, s, "sort(" + y + ")", {
			field: r,
			type: i
		})
		...
		var t = 0,
			...
			a = [],
			l = d.cache[e];
		return l ? (layui.each(l, function(e, l) {
